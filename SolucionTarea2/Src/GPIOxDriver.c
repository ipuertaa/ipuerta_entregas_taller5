...
/* El error corresponde a que, con la operación anterior sólo estoy trasladando la posición de registro deseada
 * hacia la posición menos significativa del registro. Pero, los valores que hayan a la izquierda siguen estando
 * y esos no me interesan. Cuando quiero leer un pin, sólo necesito ese pin, no el resto.
 *
 * Por lo tanto, necesito hacer una máscara para volver cero el resto de las posiciones de registro y dejar SIN
 * modificar la posición en particular que necesito.
 */

	pinValue &= (1<<0);

/* Con la máscara anterior, se garantiza que todas las posiciones de registro a la izquierda se vuelvan cero
 * y el único valor que lea sea el del pin que busco
 */
	return pinValue;
